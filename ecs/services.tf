resource "aws_ecs_service" "blockparty" {
  for_each                           = var.ecs_services
  name                               = "${each.key}-service"
  cluster                            = aws_ecs_cluster.blockparty.id
...
#  }
  ...
  depends_on = [aws_ecs_cluster.blockparty, aws_ecs_capacity_provider.spot, var.target_group_arns]
  lifecycle {
